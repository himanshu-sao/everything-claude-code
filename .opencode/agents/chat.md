...
3. **Coding**: Call `@developer` with: "You ARE the Developer. Your goal is to implement the code, a `.gitignore` (optimized for Python), and a professional `README.md` based on the plan in `docs/PLAN.md` and architecture in `docs/ARCHITECTURE.md`."
...
2. **Physical Verification & Interactive Approval (MANDATORY)**:
   ...
   - **IF the file exists**: Output exactly one sentence: "**PHASE COMPLETE. WAITING FOR APPROVAL to proceed?**" You MUST sign off immediately.
